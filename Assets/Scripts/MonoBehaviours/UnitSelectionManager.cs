    ...
    if (Input.GetMouseButtonDown(1)) {
      ...
      if (collisionWorld.CastRay(raycastInput, out Unity.Physics.RaycastHit raycastHit)) {
        if (entityManager.HasComponent<Unit>(raycastHit.Entity)) {
          ...
          if (faction.factionType == FactionType.Zombie) {
            // Right clicking a zombie
            isAttackingSingleTarget = true;

            entityQuery = new EntityQueryBuilder(Allocator.Temp).WithAll<Selected>().WithPresent<TargetOverride>().WithPresent<UnitMover>().WithPresent<LocalTransform>().Build(entityManager);
            NativeArray<Entity> entityArray = entityQuery.ToEntityArray(Allocator.Temp);
            ...
            for (int i = 0; i < targetOverrideArray.Length; i++) {
              ...
              unitMover.targetPosition = localTransformArray[i].Position;
              ...
              entityManager.SetComponentData(entity, targetOverride);
      ...
      if (!isAttackingSingleTarget) {
        ...
        for (int i = 0; i < moveOverrideArray.Length; i++) {
          ...
          // Remove target override when moving units.
          TargetOverride targetOverride = targetOverrideArray[i];
