...
partial struct ResetTargetSystem : ISystem {
  ...
  public void OnUpdate(ref SystemState state) {
    ...
    //  if (target.ValueRW.targetEntity != Entity.Null && (!SystemAPI.Exists(target.ValueRO.targetEntity) || !SystemAPI.HasComponent<LocalTransform>(target.ValueRO.targetEntity))) {
    //    target.ValueRW.targetEntity = Entity.Null;
    //  }
    //}

    //foreach (RefRW<TargetOverride> targetOverride in SystemAPI.Query<RefRW<TargetOverride>>()) {
    //  if (targetOverride.ValueRW.targetEntity != Entity.Null && (!SystemAPI.Exists(targetOverride.ValueRO.targetEntity) || !SystemAPI.HasComponent<LocalTransform>(targetOverride.ValueRO.targetEntity))) {
